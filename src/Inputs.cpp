...
void Inputs(GLFWwindow *window) {
  ...
  if (isInWindow) {
    ...
    if (glfwGetKey(window, GLFW_KEY_1) == GLFW_PRESS) {
      object_vec.emplace_back(
          std::make_unique<Circle>(Vec2(xpos, screen_height - ypos), 1, 6));
      object_vec.emplace_back(
          std::make_unique<Circle>(Vec2(xpos, screen_height - ypos), 1, 6));
      object_vec.emplace_back(
          std::make_unique<Circle>(Vec2(xpos, screen_height - ypos), 1, 6));
    ...
    if (glfwGetKey(window, GLFW_KEY_2) == GLFW_PRESS) {
      object_vec.emplace_back(
          std::make_unique<Circle>(Vec2(xpos, screen_height - ypos), 3, 9));
    }
    // Spawn balls
    if (glfwGetKey(window, GLFW_KEY_3) == GLFW_PRESS) {
      object_vec.emplace_back(
          std::make_unique<Circle>(Vec2(xpos, screen_height - ypos), 6, 15));
    }
    // Spawn balls
    if (glfwGetKey(window, GLFW_KEY_4) == GLFW_PRESS) {
      object_vec.emplace_back(
          std::make_unique<Circle>(Vec2(xpos, screen_height - ypos), 9, 25));
    }
    // Spawn balls
    if (glfwGetKey(window, GLFW_KEY_5) == GLFW_PRESS) {
      object_vec.emplace_back(
          std::make_unique<Circle>(Vec2(xpos, screen_height - ypos), 40, 40));
    }
  }
}
...
void mouseButtonCallback(GLFWwindow *window, int button, int action, int mods) {
  if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS && isInWindow) {
    object_vec.emplace_back(
        std::make_unique<Circle>(Vec2(xpos, screen_height - ypos), 50, 50));
